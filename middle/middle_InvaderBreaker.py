...
def resource_path(relative_path):
    """ PyInstaller용 리소스 경로 보정 함수 """
    try:
        # PyInstaller에 의해 임시폴더(_MEIPASS)가 생성되면 그 경로를 사용
        base_path = sys._MEIPASS
    except Exception:
        # 일반 실행 시에는 현재 파일의 폴더 경로를 사용
...
def main():
    # --- 배경음악 로드 및 재생 ---
    try:
        pygame.mixer.music.load(resource_path("sounds/bgmusic.mp3"))
    ...
    try:
        bullet_img = pygame.image.load(resource_path("sprites/arrow_right.png")).convert_alpha()
    ...
    try:
        ...
        item_img_raw = pygame.image.load(resource_path("sprites/suit_diamonds.png")).convert_alpha()
    ...
    # --- [추가] 효과음 로드 ---
    try:
        shoot_sound = pygame.mixer.Sound(resource_path("sounds/gunFire.mp3"))
    ...
    # --- [추가] 튕김 효과음 로드 ---
    try:
        bounce_sound = pygame.mixer.Sound(resource_path("sounds/slight_boink.wav"))
